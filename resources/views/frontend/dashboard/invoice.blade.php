                        ...
                        <div class="fp_dashboard_body rounded bg-white p-5 shadow">
                            ...
                            <div class="row mb-4">
                                <div class="col-md-6">
                                    ...
                                    <p><strong>Subtotal:</strong> {{ currencyPosition(number_format($order->subtotal, 2)) }}</p>

                                    @if ($order->discount)
                                        <p><strong>Discount:</strong> {{ currencyPosition(number_format($order->discount, 2)) }}</p>
                                    @endif

                                    <p><strong>Total:</strong> {{ currencyPosition(number_format($order->total, 2)) }}</p>
                                </div>
                                <div class="col-md-6">
                                ...
                                <tbody>
                                    @foreach ($order->items as $item)
                                        <tr>
                                            <td>{{ $item->name }}</td>
                                            <td>{{ $item->qty }}</td>
                                            <td>{{ currencyPosition(number_format($item->price, 2)) }}</td>
                                            <td>{{ currencyPosition(number_format($item->price * $item->qty, 2)) }}</td>
                                        </tr>
                                    @endforeach
                                </tbody>
                                <tfoot>
                                    <tr>
                                        ...
                                        <th>{{ currencyPosition(number_format($order->total, 2)) }}</th>
                                    </tr>
                                </tfoot>
                            </table>
